...
export default function SuscripcionesScreen() {
  ...
  // Conteo de alertas (por vencer + vencidas)
  ...
  const filteredSuscripciones = useMemo(() => {
    ...
    if (debouncedSearch.trim()) {
      ...
      result = result.filter((s) => {
        const socio = nombreParticipante(suscripcionSocio(s)).toLowerCase();
        const plan = s.suscripcion_detalle?.nombre?.toLowerCase() || '';
        return socio.includes(term) || plan.includes(term);
      });
    }
    ...
    // Ordenar por días hasta vencimiento (vencidas y por vencer primero)
    return [...result].sort(
      (a, b) => diasHastaVencimiento(a) - diasHastaVencimiento(b)
    );
